...
# About Schemas
Any time a plugin needs to store its own data, or needs to ask the user for data, the Delphix Engine needs to be told about the format of that data: How many pieces of data are needed? What should they be called? Are they strings? Integers? Booleans?
...
2. It is used to generate a custom user interface, and to validate user inputs.
3. It is used to [autogenerate Python classes](#autogenerated-classes) that can be used by plugin code.

There are five plugin-customizable data formats, one each for [repositories](/References/Glossary/#repository), [source configs](/References/Glossary/#source-config), [linked sources](/References/Glossary/#linked-source), [virtual sources](/References/Glossary/#virtual-source), and [snapshots](/References/Glossary/#snapshot).

# JSON Schemas

Plugins use a JSON schemas for their custom datatypes. There are three main things to understand about them, which are explained just below:
...
But, this isn't *just* a JSON object. This is a JSON schema. It uses the special keywords like `type` `required`, and `additionalProperties`. These have specially-defined meanings in the context of JSON schemas.
...
For example, suppose the engine we already know about a single repository with data `{"dbname": "my_databsae", "path": "/var/db/db01"}` (note the misspelling!). And, then suppose that automatic discovery is re-run and it returns repository data `{"dbname": "my_database", "path": "/var/db/db01"}`. What should the Delphix Engine do? Should it conclude that "my_databsae" has been deleted, and there is a completely new repository now named "my_database"? Or, should it conclude that we still have the same old repository, but with an updated name?
...
The `nameField` keyword specifies a single property that is to be used to name the object in the Delphix Engine. The property must be a string field. This keyword is used at the same level as `properties`. It is **required** for repository and source config schemas, and may not be used in any other schemas.
...
So, if we have an repository object that looks like `{"path":"/usr/bin", "port": 8800}`, then the user will be able to refer to this object as `/usr/bin`.


### `password`

The `password` keyword can be used to specify the `format` of a `string`. (Note that `format` is a standard keyword and is not Delphix-specific). If a property is tagged as a password, then the UI will never show the value on screen, and the value will be encrypted before being stored.
