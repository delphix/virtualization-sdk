...
def staged_pre_snapshot(staged_source, repository, source_config):
  pass
```
...
def staged_post_snapshot(staged_source, repository, source_config):
  snapshot = SnapshotDefinition()
  snapshot.transaction_id = 1000
  return snapshot
```
...
def staged_start_staging(staged_source, repository, source_config):
  pass
```
...
def staged_stop_staging(staged_source, repository, source_config):
  pass
```
...
def staged_worker(staged_source, repository, source_config):
  pass
```
...
def mount_specification(staged_source, repository):
  mount = SingleMount()
