...
Defines properties for the target physical source when converting a virtual or linked source to a physical source.

### VirtualToPhysicalDefinition Schema

The `virtualToPhysicalDefinition` is a **required** schema in `schema.json`. It can be an empty object (`{}`) if no additional user input is needed for the physical conversion operation.
...
### VirtualToPhysicalDefinition Class

Autogenerated based on the [VirtualToPhysicalDefinition Schema](#virtualtophysicaldefinition-schema).

```python
class VirtualToPhysicalDefinition:

  def __init__(self, target_directory=None):
    self._inner_dict = {"targetDirectory": target_directory}
```
...
# Construct with an optional target directory
vtop = VirtualToPhysicalDefinition(target_directory="/mnt/physical_target")

# Access the property
target_dir = vtop.target_directory
```
